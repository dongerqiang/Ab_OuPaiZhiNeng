package com.opa.android.mode.utils;
...
public class ParserData {
	...
	public int parserMileage(byte data[]){
		...
		int km = (int) (MyApplication.app.deviceNotes.getWheel()* huoerCount / 10.0f ) ;
	...
	public int parserSpeed(byte data[]){
		...
		if(yadiSpeedHallCount == 0){
			speedKm = 1.2f*(speedHallCount*circumference/1000f)/(6f*MyApplication.app.deviceNotes.optMotorJds(false, 1));
		}else{
			speedKm = 1.2f*(yadiSpeedHallCount*circumference/1000f)/6f/MyApplication.app.deviceNotes.optMotorJds(false, 1);
		}
		    ...
		    float percent = 0;
		       switch(type) {
		        ...
		        case 48:
		               ...
		               return percent;
		        case 60:
		               if (battery>=52&&battery<=73.5) {
		                   percent = (battery-52.0f)/(73.5f-52.0f);
		               }
		               return percent;
		        case 64:
